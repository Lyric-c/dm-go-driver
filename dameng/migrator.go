...
func (m Migrator) MigrateColumn(dst interface{}, field *schema.Field, columnType gorm.ColumnType) error {
	...
	if alterColumn && !field.IgnoreMigration {
		return m.alterColumn(dst, field.DBName, containsUnique)
	}
...
(SELECT NAME, ID FROM SYS.SYSCOLUMNS WHERE UPPER(NAME) = UPPER(?)) COLS
...
(SELECT ID, PID, NAME FROM SYS.SYSOBJECTS WHERE SUBTYPE$='INDEX' AND UPPER(NAME) = UPPER(?)) OBJ_INDS
...
WHERE TAB.ID = COLS.ID AND TAB.ID = OBJ_INDS.TABLEID AND COLS.COLID = OBJ_INDS.COLID AND TAB.SCHID = USERS.ID AND UPPER(OBJ_INDS.NAME) = UPPER(?))`
